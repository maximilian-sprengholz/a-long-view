...
if "`outfmt'"=="HTML" {
	...
	global tabout_ttlpre ///
		style(htm) noplines nohlines noborder ntc ///
		topf(${dir_do}auxiliary/tabout_topf_detail.txt) topstr(
	global tabout_ttlpost ///
		| scientific medleftstub) botf(${dir_do}auxiliary/tabout_botf_detail.txt)
}
else if "`outfmt'"=="MD" {
	global gsfx ".svg"
	global tsfx ".md"
	...
	global tabout_ttlpre ///
		...
		topf(${dir_do}auxiliary/tabout_topf_detail_md.txt) topstr(
	global tabout_ttlpost ///
		|scientific medleftstub) botf(${dir_do}auxiliary/tabout_botf_detail_md.txt)
}
else if "`outfmt'"=="WORD" {
	global gsfx ".emf"
	global tsfx ".docx"
...
preserve
	...
	qui forvalues s=1/2 {
		...
		local tblttl "Education (ISCED-97) by cohort and arrival age groups, `lbl`s'' (duration of stay 1-6)"
...
local ylbl_empl_dummy 	0.2(0.1)1
	...
	preserve
		...
		forvalues s=1/2 {
			...
			twoway ///
				...
				title("`ttl', `lbl`s''") subtitle("Means, age 18-54") ///
				xlabel(0(5)31) ///
				legend(order(- "Arr. cohort: " 1 "1964-73" 2 "1974-83" 3 "1984-93" ///
				4 "1994-03" 5 "2004-10") cols(6) stack span)) ///
...
preserve
	...
	*/
	keep if year>=2007
	recode cship_nat (.=.) (0=.) (3=2) (4 2 = 3) (0 -5 = 1), gen(nat_c)
	...
	forvalues s=1/2 {
		forvalues e=1/3 {
			...
			twoway ///
				...
				title("`ttl', `lbl`s''") subtitle("Shares") ///
				xlabel(1976(5)2016) /*
